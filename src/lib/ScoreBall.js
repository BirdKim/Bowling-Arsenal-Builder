...
// Finish specs
export const FINISH_FRICTION_SAND = { "500": 100, "1000": 80, "1500": 68, "2000": 60, "3000": 48, "4000": 39, "4500": 36, "5000": 33, "5500": 30 };
export const FINISH_FRICTION_POLISH = { "Compound": 10, "Polish": 15, "Reacta Gloss": 20 };
// Coverstock specs
...
export const BACKEND_RESPONSE = { "Plastic": 0, "Urethane": 20, "Urethane Solid Reactive": 40, "Pearl Reactive": 90, "Hybrid Reactive": 65, "Solid Reactive": 50 };
...
export function scoreBall(ball) {
  ...
  const finishFrictionSand = FINISH_FRICTION_SAND[ball.finish_grit] ?? 55;
  const finishFrictionPolish = FINISH_FRICTION_POLISH[ball.finish_polish] ?? 15;
  const finishFriction = finishFrictionSand - finishFrictionPolish;  // higher = more friction potential
  const coverTraction = COVER_TRACTION[ball.coverstock_type] ?? 60;
  const coverEnergyRetention = ENERGY_RETENTION[ball.coverstock_type] ?? 50;
  const coverBackendResponse = BACKEND_RESPONSE[ball.coverstock_type] ?? 50;

  const length = clamp(
    coverEnergyRetention * 0.15 +
    rgScore * 0.50 +
    (100 - finishFriction) * 0.20 +
    (100 - coverTraction) * 0.15
  );
  ...
  const backendShape = clamp(
    coverBackendResponse * 0.15 +
    massBiasScore * 0.55 +
    coverEnergyRetention * 0.20 +
    finishFriction * 0.10
  );

  // const continuation = clamp(
  //   coverEnergyRetention * 0.40 +
  //   diffScore * 0.30 +
  //   coverBackendResponse * 0.20 +
  //   massBiasScore * 0.10
  // );
  ...
  return {
    ...
    finishFriction,
    coverTraction,
    coverEnergyRetention,
    coverBackendResponse,
  };
}